...
public class ChessBoard {
    ...
    public boolean equals(Object otherBoard) {
        if (this == otherBoard) {
            ...
            return false;
        } else if (otherBoard.getClass() != this.getClass()) {
            return false;
        }
        ChessBoard that = (ChessBoard) otherBoard;
        for (int i = 1; i < 9; i++) {
            for (int j = 1; j < 9; j++) {
                if (this.getPiece(new ChessPosition(i, j)) == null) {
                    if (that.getPiece(new ChessPosition(i, j)) != null) {
                        return false;
                    }
                } else if (!this.getPiece(new ChessPosition(i, j)).equals(that.getPiece(new ChessPosition(i, j)))) {
                    return false;
                }
            }
        }
